...
export default function Dashboard() {
  ...
  // Compile active simplified settlements (debts) across all groups for dashboard listing
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col font-mono relative overflow-hidden">
      ...
      {/* Hero Header */}
      <header className="mx-auto w-full max-w-7xl px-4 py-8 md:px-8 relative z-10">
        <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between border-b border-primary/20 pb-6">
          <div>
            <div className="text-[9px] text-primary uppercase tracking-[0.4em] font-bold mb-1.5 flex items-center gap-1.5">
            ...
            <h1 className="text-3xl font-black italic tracking-tighter text-white uppercase">
              WELCOM_BACK // <span className="text-primary neon-primary">{currentUser.name.split(' ')[0]}</span>
            </h1>
            <p className="text-gray-500 text-xs mt-1.5 max-w-md uppercase tracking-wider">
          ...
          <div className="flex flex-wrap gap-2.5">
            <button
              ...
              className="flex items-center gap-2 border border-secondary/40 bg-black px-4 py-2 text-xs font-bold text-secondary hover:bg-secondary/5 transition-all cursor-pointer uppercase tracking-wider"
            >
              [+] ADD_FRIEND_NODE
            </button>
            <button
              onClick={() => setShowJoinModal(true)}
              className="flex items-center gap-2 border border-tertiary/40 bg-black px-4 py-2 text-xs font-bold text-tertiary hover:bg-tertiary/5 transition-all cursor-pointer uppercase tracking-wider"
            >
              [*] CONNECT_GROUP
            </button>
            <button
              onClick={() => setShowCreateModal(true)}
              className="flex items-center gap-2 bg-primary px-4 py-2 text-xs font-black text-black hover:bg-white transition-all shadow-[0_0_15px_rgba(255,211,0,0.3)] cursor-pointer uppercase tracking-wider"
            >
              [+] INITIALIZE_GROUP
            </button>
          </div>
      ...
      <main className="mx-auto w-full max-w-7xl flex-1 px-4 pb-16 md:px-8 space-y-8 relative z-10">
        ...
        <section className="grid gap-6 md:grid-cols-3">
          ...
          <div className="hud-card rounded-none p-5 flex flex-col justify-between relative overflow-hidden min-h-[140px]">
            <div>
              <p className="text-[9px] font-bold uppercase tracking-widest text-primary">// NET_LEDGER_BALANCE</p>
              <h3 className={`text-3xl font-black mt-3 italic tracking-tight ${netTotal >= 0 ? 'text-primary neon-primary' : 'text-error text-shadow-error'}`}>
                {netTotal >= 0 ? `+ ₹${netTotal.toFixed(2)}` : `- ₹${Math.abs(netTotal).toFixed(2)}`}
              </h3>
            </div>
            <div className="text-[9px] text-gray-500 uppercase mt-4 tracking-wider">
          ...
          <div className="hud-card rounded-none p-5 flex items-start gap-4 border-l-4 border-l-primary">
            ...
            <div>
              <p className="text-[9px] font-bold uppercase tracking-widest text-gray-400">// CREDITOR_MATRIX</p>
              <h3 className="text-2xl font-black mt-2 text-white">₹{totalOwedToMe.toFixed(2)}</h3>
              <p className="text-[9px] text-gray-500 mt-1.5 uppercase">Owed by other network nodes</p>
            </div>
          </div>
          ...
          <div className="hud-card rounded-none p-5 flex items-start gap-4 border-l-4 border-l-error">
            ...
            <div>
              <p className="text-[9px] font-bold uppercase tracking-widest text-gray-400">// DEBTOR_MATRIX</p>
              <h3 className="text-2xl font-black mt-2 text-white">₹{totalIOwe.toFixed(2)}</h3>
              <p className="text-[9px] text-gray-500 mt-1.5 uppercase">Requires settlement clearance</p>
            </div>
          </div>
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-5">
            <div className="flex items-center justify-between border-b border-primary/20 pb-3">
              <h2 className="text-sm font-bold tracking-widest text-white uppercase flex items-center gap-2">
                <Users className="h-4.5 w-4.5 text-secondary" /> ACTIVE_SECTOR_GROUPS
              </h2>
              <span className="text-[9px] text-secondary font-bold bg-secondary/10 border border-secondary/30 px-2 py-0.5">
            ...
            {userGroups.length === 0 ? (
              <div className="hud-card rounded-none p-12 text-center border-dashed border-primary/20">
                <Users className="mx-auto h-10 w-10 text-primary/40 animate-pulse" />
                <h3 className="mt-4 text-xs font-bold uppercase tracking-widest text-white">No active network groups</h3>
                <p className="mt-2 text-[10px] text-gray-500 max-w-xs mx-auto uppercase">
                  Boot up a group to share room bills, rent ledger, or Goa trip files.
                </p>
                <div className="mt-6 flex justify-center gap-2">
                  <button
                    onClick={() => setShowJoinModal(true)}
                    className="border border-secondary/40 bg-black text-secondary hover:bg-secondary/5 px-4 py-2 text-[10px] font-bold cursor-pointer uppercase"
                  >
                    Connect via code
                  </button>
                  <button
                    onClick={() => setShowCreateModal(true)}
                    className="bg-primary text-black hover:bg-white px-4 py-2 text-[10px] font-bold cursor-pointer uppercase"
                  >
                    Create channel
                  </button>
                </div>
                  ...
                  >
                    <div>
                      <div className="flex items-start justify-between gap-4">
                        <h3 className="font-black text-white text-sm group-hover:text-primary transition-colors uppercase tracking-tight">
                          {group.name}
                        </h3>
                        <span className="text-[8px] font-bold text-secondary bg-secondary/5 border border-secondary/20 px-1.5 py-0.5">
                          {group.members.length} NODES
                        </span>
                      </div>
                      {group.description && (
                        <p className="text-[10px] text-gray-500 mt-2 lowercase leading-relaxed line-clamp-2">
                          // {group.description}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="mt-6 pt-4 border-t border-primary/10 flex items-center justify-between">
                      <div>
                        <p className="text-[8px] font-bold uppercase tracking-widest text-gray-500">Node Balance</p>
          ...
          <div className="space-y-6">
            ...
            {/* Settlements to pay */}
            <div>
              <h2 className="text-sm font-bold tracking-widest text-white mb-3 uppercase flex items-center gap-2 border-b border-primary/20 pb-3">
                <Send className="h-4.5 w-4.5 text-primary" /> CLEARANCE_MATRIX
              </h2>

              {pendingPaymentsToMake.length === 0 ? (
                <div className="hud-card rounded-none p-5 text-center text-[10px] text-gray-500 uppercase tracking-wider">
                  ...
                  {pendingPaymentsToMake.map((debt, idx) => (
                    <div key={idx} className="hud-card rounded-none p-4 flex flex-col justify-between gap-3">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="text-[8px] text-gray-500 font-bold uppercase">{debt.groupName}</p>
                          <p className="text-xs font-bold text-white mt-1 uppercase">
                            Pay Node <span className="text-secondary">{debt.payeeName}</span>
                          </p>
                        </div>
                        <span className="text-xs font-black text-white">₹{debt.amount.toFixed(2)}</span>
                      </div>
                      ...
                      {debt.payeeUpi ? (
                        <button
                          ...
                          className="w-full flex items-center justify-center gap-1.5 border border-primary/40 hover:border-primary bg-primary/5 hover:bg-primary text-primary hover:text-black py-1.5 text-[10px] font-bold transition-all cursor-pointer uppercase tracking-wider"
                        >
                          <QrCode className="h-3.5 w-3.5" /> Settle via UPI
                        </button>
                      ) : (
                        <div className="border border-error/20 bg-error/5 p-2 text-[9px] text-error flex items-center gap-1">
            ...
            <div>
              <h2 className="text-sm font-bold tracking-widest text-white mb-3 uppercase flex items-center gap-2 border-b border-primary/20 pb-3">
                <UserCheck className="h-4.5 w-4.5 text-tertiary" /> DISCHARG_APPROVALS
              </h2>
                    ...
                    <div key={settlement.id} className="hud-card rounded-none p-4 border border-tertiary/30 bg-tertiary/5 space-y-3">
                      <div className="flex items-center gap-3">
                        {settlement.senderAvatar ? (
                          ...
                          <div className="h-7 w-7 bg-muted flex items-center justify-center text-[9px]">
                            👤
                          </div>
                        )}
                        <div className="flex-1 min-w-0">
                          <p className="text-[8px] text-gray-500 font-bold uppercase truncate">{settlement.groupName}</p>
                      ...
                      <div className="flex justify-between items-center bg-black rounded-none px-3 py-1.5 border border-primary/10 font-mono">
                        <span className="text-[9px] text-gray-500">Amount:</span>
                        <span className="text-xs font-bold text-primary">₹{settlement.amount.toFixed(2)}</span>
                      </div>

                      {settlement.upiTxnRef && (
                        <div className="text-[8px] text-gray-500 pt-1.5 flex justify-between border-t border-primary/5 uppercase">
                          <span>UPI_REF_TXN:</span>
                          <span className="text-gray-300 font-mono">{settlement.upiTxnRef}</span>
                        </div>
                      )}
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex-1 flex items-center justify-center gap-1 border border-error bg-error/5 text-error text-[9px] py-1.5 font-bold hover:bg-error hover:text-black transition-colors cursor-pointer uppercase"
                        >
                          <XCircle className="h-3.5 w-3.5" /> Decline
                        </button>
                        <button
                          onClick={() => confirmSettlement(settlement.id)}
                          className="flex-1 flex items-center justify-center gap-1 bg-primary text-black text-[9px] py-1.5 font-black hover:bg-white transition-colors cursor-pointer uppercase"
                        >
      ...
      {activePaySettlement && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-xs">
          <div className="hud-card max-w-sm w-full rounded-none p-6 text-center space-y-6 relative border-2 border-primary screen-glow">
            ...
            <div className="space-y-1.5 text-[10px] uppercase font-mono">
              ...
              <div className="bg-black border border-primary/20 p-2 font-mono text-gray-300 break-all select-all">
                {activePaySettlement.upiId}
              </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-xs">
          <form onSubmit={handleCreateGroup} className="hud-card max-w-md w-full rounded-none p-6 space-y-4 relative border-2 border-primary screen-glow">
            ...
            <div className="space-y-1.5 text-xs">
              <label className="text-[9px] font-bold text-gray-500 uppercase tracking-wider">Channel Name</label>
              <input
                type="text"
            ...
            <div className="space-y-1.5 text-xs">
              <label className="text-[9px] font-bold text-gray-500 uppercase tracking-wider">Node Description (Optional)</label>
              <textarea
            ...
            <div className="flex gap-2 pt-2">
              <button
                ...
                className="flex-1 border border-gray-700 hover:border-gray-500 text-gray-400 text-xs py-2 font-bold cursor-pointer uppercase"
              >
                Abort
              </button>
              <button
      ...
      {showJoinModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-xs">
          <form onSubmit={handleJoinGroup} className="hud-card max-w-sm w-full rounded-none p-6 space-y-4 relative border-2 border-primary screen-glow">
            ...
            <p className="text-[9px] text-gray-500 uppercase">Input the 6-character node sequence shared by the host.</p>
            
            <div className="space-y-1.5 text-xs">
              <label className="text-[9px] font-bold text-gray-500 uppercase tracking-wider">Node Invite Code</label>
              <input
                type="text"
      ...
      {showUserModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-xs">
          <form onSubmit={handleAddFriend} className="hud-card max-w-md w-full rounded-none p-6 space-y-4 relative border-2 border-primary screen-glow">
            ...
            <p className="text-[9px] text-gray-500 uppercase">Deposit a local simulated user onto your workspace grid.</p>
            
            <div className="space-y-1.5 text-xs">
              <label className="text-[9px] font-bold text-gray-500 uppercase tracking-wider">Node / Friend Alias</label>
              <input
                type="text"
            ...
            <div className="space-y-1.5 text-xs">
              <label className="text-[9px] font-bold text-gray-500 uppercase tracking-wider">Node Email Identifier</label>
              <input
                type="email"
            ...
            <div className="space-y-1.5 text-xs">
              <label className="text-[9px] font-bold text-gray-500 uppercase tracking-wider">Node UPI Address (Receiving)</label>
              <input
                type="text"
